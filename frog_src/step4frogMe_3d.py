...
for mol in tqdm(suppl):
    ...
    for frag_smiles in brics_frags:
        ...
        if rule_of_3(frag_mol):
            # Only add explicit hydrogens and embed if there are NO dummy atoms
            if not any(atom.GetAtomicNum() == 0 for atom in frag_mol.GetAtoms()):
                frag_mol = Chem.AddHs(frag_mol)
                try:
                    AllChem.EmbedMolecule(frag_mol)
                    AllChem.MMFFOptimizeMolecule(frag_mol)
                except Exception:
                    pass  # Accept fragments even if optimization fails
           # Proceed to store all fragments (including with dummy atoms) for output/enumeration
